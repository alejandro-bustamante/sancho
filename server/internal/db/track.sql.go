...
RETURNING id, title, artist_id, album_id, duration, track_number, disc_number, sample_rate, bit_depth, bitrate, channels, codec, file_path, file_size, isrc, created_at
`
...
SELECT id, title, artist_id, album_id, duration, track_number, disc_number, sample_rate, bit_depth, bitrate, channels, codec, file_path, file_size, isrc, created_at FROM track
WHERE LOWER(title) LIKE LOWER('%' || ?1 || '%')
ORDER BY created_at DESC
`

func (q *Queries) SearchTracksByTitle(ctx context.Context, query sql.NullString) ([]Track, error) {
	rows, err := q.query(ctx, q.searchTracksByTitleStmt, searchTracksByTitle, query)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
